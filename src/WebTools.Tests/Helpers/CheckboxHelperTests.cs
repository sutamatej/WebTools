...
namespace WebTools.Tests.Helpers
{
    public class CheckboxHelperTests
    {
        ...
        public void Checkbox_helper_supports_attribute_combinations()
        {
            ...
            var result = helper.CheckBox(m => m.SomeProperty).Class("test-class").Id("test-id").ToString();

            Assert.Equal("<input checked=\"checked\" class=\"test-class\" id=\"test-id\" name=\"SomeProperty\" type=\"checkbox\" value=\"true\" /><input name=\"SomeProperty\" type=\"hidden\" value=\"false\" />", result);
        }
    }
